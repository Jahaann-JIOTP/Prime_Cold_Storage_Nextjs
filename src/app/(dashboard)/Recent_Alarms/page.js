...
const Page = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        const response = await axios.get(
          `http://15.206.128.214/Test_Api/comparison2.php?filter=${filter}`
        );
  ...
  return (
    <div className="relative w-full  shadow-lg h-[85vh] rounded-lg border-t-4 border-t-[#1F5897] overflow-auto border-2 border-[#808080]">
      <div className="absolute inset-0 bg-[#f2f2f2]" style={{ opacity: 0.5 }}></div>
      {/* Header */}
      <div className="p-4  border-b border-white flex justify-between items-center mb-6 relative z-10">
        <p className="text-lg font-bold text-black">
          Alarms History - Recent Alarms
        </p>
        <div className="relative">
          <select
            className="block w-full px-4 py-2 text-sm border border-white rounded shadow focus:outline-none focus:ring-2 focus:ring-blue-500"
            value={filter}
      ...
      <div className="overflow-x-auto relative z-10">
        <table className="min-w-full table-auto border-collapse border border-white">
          <thead>
            <tr className="bg-blue-50 border-b border-white">
