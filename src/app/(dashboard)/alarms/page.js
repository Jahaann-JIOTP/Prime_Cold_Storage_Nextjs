...
export default function Home() {
  ...
  const submitData = async () => {
    ...
    const dataToSubmit = {
      Source: selectedMeter, // Ensure this is set to the correct value
      Status: selectedOption, // Ensure this is set to the correct option
      value: inputValue,
      Time: new Date().toISOString(), // Send the current timestamp in UTC
    };
    ...
    try {
      ...
      if (result.success) {
        ...
        fetchData(); // Re-fetch data after successfully submitting
      } else {
  ...
  return (
    <div className="flex-shrink-0 w-full h-[85vh] shadow-lg rounded-lg border-t-4 border-t-[#1F5897] overflow-hidden border-2 border-[#808080]">
      ...
      <div className="mt-8 px-8">
        <button
          className=" text-white font-medium py-3 px-6 rounded-lg shadow-md hover:from-[#0B8240] hover:to-[#10532e] transition-all duration-300 ease-in-out transform hover:scale-105"
          onClick={toggleDropdown}
          style={{
            background: "linear-gradient(#0B8240,#0B8240)",
        ...
        {isMeterDropdownVisible && (
          <div className="bg-white text-gray-700 shadow-2xl mt-3 py-4 px-4 rounded-lg overflow-y-auto h-64 transform transition-all duration-300 ease-in-out hover:scale-105 hover:shadow-2xl hover:bg-[#F7FAFC]">
            ...
            <a
              href="#"
              className="block py-3 px-4 hover:bg-[#E2E8F0] rounded-md transition-all duration-200"
              onClick={() => showMeterValue("Auto Packing")}
            >
              Auto Packing
            </a>
            <a
