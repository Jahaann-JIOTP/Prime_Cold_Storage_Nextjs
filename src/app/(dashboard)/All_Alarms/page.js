...
const AlarmTable = () => {
  ...
  useEffect(() => {
    const fetchData = () => {
      axios
        .get("http://15.206.128.214/Test_Api/comparison.php") // Update to your backend API URL
        .then((response) => {
  ...
  return (
    <div className="relative w-full  shadow-lg h-[85vh] rounded-lg border-t-4 border-t-[#1F5897] overflow-hidden border-2 border-[#808080] ">
      {/* Header */}
      <div
        className="absolute inset-0 bg-[#f2f2f2]"
        style={{ opacity: 0.5 }}
      ></div>
      <div className="p-4  border-b border-white relative z-10">
      ...
      {/* Table */}
      <div className="overflow-x-auto h-[75%] px-4 z-10 relative">
        <table className="min-w-full table-auto border-collapse border border-white">
          <thead className="bg-blue-50 border-b border-white">
            <tr>
      ...
      {/* Pagination */}
      <div className="p-4 flex justify-between items-center mt-[-20px]">
        <div>
          <button
            onClick={() => handlePageChange(1)}
            disabled={currentPage === 1}
            className={`px-4 py-2 mr-2 rounded-md border border-blue-300 
    bg-gradient-to-b from-blue-100 to-blue-300 text-blue-900 
    shadow-[inset_0_1px_3px_rgba(255,255,255,0.6)] 
    backdrop-blur-md 
